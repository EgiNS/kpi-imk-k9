...
const BeritaTerkini = () => {
  ...
  const currentItems = DataBerita.slice(indexOfFirstItem, indexOfLastItem);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto mt-8">
      <h1 className="font-bold text-[#420101] md:text-3xl text-2xl mb-6">Berita Terkini</h1>
      <h3 className="font-medium text-[#000000] md:text-sm text-sm mb-2">
        Menampilkan {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, DataBerita.length)} dari {DataBerita.length} Berita Terkini
      </h3>
          ...
          <div key={berita.id} className="p-4 rounded-2xl shadow grid sm:grid-cols-6 grid-cols-1 sm:grid-rows-1 grid-rows-2 gap-x-3 bg-[#EDDBD9]">
            <div style={{backgroundImage: `url(${berita.img})`}} className="sm:col-span-2 bg-cover bg-center"/>
              <div className='sm:col-span-4'>
                <Link to={`/terkini/${berita.id}`} className="text-lg font-semibold hover:underline">{berita.title}</Link>
